...
def match(threshold, truths, priors, variances, labels, landms, loc_t, conf_t, landm_t, idx):
    ...
    best_prior_overlap = overlaps.max(1, keepdim=True).numpy()
    best_prior_idx = overlaps.argmax(1, keepdim=True).numpy()

    # ignore hard gt
    valid_gt_idx = best_prior_overlap[:, 0] >= 0.2
    best_prior_idx_filter = best_prior_idx[valid_gt_idx, :]
    best_prior_idx_filter = paddle.to_tensor(best_prior_idx_filter, stop_gradient=False)
    best_prior_overlap = paddle.to_tensor(best_prior_overlap, stop_gradient=False)
    best_prior_idx = paddle.to_tensor(best_prior_idx, stop_gradient=False)
    ...
    best_truth_overlap = overlaps.max(0, keepdim=True)
    best_truth_idx = overlaps.argmax(0, keepdim=True)
    ...
    for i in best_prior_idx_filter:
        best_truth_overlap[i] = 2
    best_truth_overlap = paddle.to_tensor(best_truth_overlap, stop_gradient=False)
    ...
    conf_t[idx] = conf.astype('int64')  # [num_priors] top class label for each prior
    landm_t[idx] = landm
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    keep = paddle.zeros(scores.shape[0], dtype='int64')
    if boxes.numel() == 0:
        return keep
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
