...
class RetinaFace(nn.Layer):
    def __init__(self, cfg = None, phase = 'train'):
        ...
        self.phase = phase
        self.backbone = resnet50(pretrained=cfg['pretrain'])
    ...
    def forward(self,inputs, epoch):
        if epoch==1:
            print(1)
        out = self.backbone(inputs, epoch)
        if epoch==1:
            print(2)
        # FPN
        fpn = self.fpn(out)
        ...
        if self.phase == 'train':
            ...
            output = (bbox_regressions, self.softmax1(classifications, axis=-1), ldm_regressions)
        return output